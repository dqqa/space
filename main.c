...
#define SPACESHIP_STICKINESS_FACTOR 0.1f
...
// TODO: Generate stars right after app start.
void update_stars(const Vector2 *speed, float dt)
{
    ...
    for (size_t layer = 0; layer < ARR_LEN(stars); layer++)
    {
        ...
        for (size_t i = 0; i < ARR_LEN(stars[0]); i++)
        {
            ...
            if (new_pos.x <= 0 || new_pos.x >= WINDOW_WIDTH || new_pos.y <= 0 || new_pos.y >= WINDOW_HEIGHT)
            {
                if (new_pos.x <= 0)
                {
